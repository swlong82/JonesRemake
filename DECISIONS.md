...
- Decision: option 2, with both halves driven by content, not code. `rules.happiness.decayPerWeek` (new, default 0 — packs that do not set it keep monotonic happiness) is applied in `core-decay` at turn start from week 2, clamped to `happiness.min`; classic sets 3, matching the dependability decay. `concert-ticket` and `theatre-ticket` gain `oncePerTurn: true`, the flag the schema and the `BuyItem` handler already honoured for the soft drink, so bulk-buying tickets no longer converts money into happiness at will.
- Consequences: happiness stops being free. Relaxing at home (+2) no longer quite covers the decay (−3), so holding happiness needs a comfort durable or a ticket, which puts happiness in competition with the wealth goal for money — the tension the goal was missing. Measured at goals 50 Normal×2 over 120 games: happiness is the last goal completed in 10.9% of games (was 0%), the stall rate stays at 0.83% and seat bias at 48.7%, and the median game grows from 44 to 47 weeks. A decay of 2 was measured as well and left happiness last in only 3.4%, so 3 is the value that meets the 9.3 target. This amends GDD 4.8's decay list for `classic`; the golden replays in `packages/engine/test/golden/` are regenerated, since the mechanic changes every seeded game.
