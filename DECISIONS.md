...
## ADR-0034: The modern AI needed a wellbeing slope, not a staircase — and a reason to buy the gadget

- Date: 2026-09-22
- Status: Accepted
- Context: M6.3's first `modern-western` measurement stalled **100% of games**: no seed produced a winner at goals 50 within 300 weeks, every seat ended bankrupt, and seats collapsed 10–15 times a game. The cause was in the AI, not the content. `wellbeing` scored a flat −1 below the collapse band, a flat −0.5 below the burnout band and a positive term above it, so inside a band the stat had no gradient at all: a rest that moved a seat from 13 to 21 scored exactly the same as not resting, while one point at the band edge was worth 0.75. A beam search cannot climb a staircase it cannot see, so seats parked just under the burnout edge and refused to work, because a single session (−3) could trip the step. They earned nothing, fell behind on rent in week 4 and never recovered — a modern seat worked 0.8 sessions a week against classic's 5.5 on the same job. `classic` never showed any of it: it has no wellbeing slice, so the scorer returns 0.
- Options: 1) tune content only and accept the plateau; 2) special-case "rest until out of burnout" in the planner; 3) make the scorer continuous and strictly increasing in the stat.
- Decision: option 3, plus one pre-ranking term. `wellbeing` is now a positive term capped at the personality's floor (0.15 at the cap) plus two linear ramps, one from the burnout edge and a steeper one from the collapse edge, so every point of the stat is worth something, the penalty near collapse is severe, and banking wellbeing above the burnout band is nearly worthless — a seat rests to stay out of trouble rather than treating wellbeing as a fifth goal. Separately, `quickScore` now adds 0.35 for buying an item with `unlocks` the seat does not own: without it the modern AI never bought a phone or laptop, so ride-hail, delivery and online study stayed shut and two whole event families could never reach a player. Option 2 was rejected for ADR-0014's reason: the planner stays a generic search and each system speaks to it only through a scorer.
- Consequences: modern collapses per game fall from 10–15 to 0–2 and the ruleset became winnable at all — with the M6.3 content tuning (ADR-0035) the same suite goes from 100% stalls and no median to 0% stalls and a 46-week median. `classic` is unaffected by construction and was re-measured to confirm it (`pnpm sim:gate`, 18 assertions, unchanged). `modern.test.ts` now asserts the score never decreases in the stat and strictly increases below the floor, so a later edit cannot reintroduce a plateau.

## ADR-0035: Modern balance is a different economy, not classic with extra systems

- Date: 2026-09-22
- Status: Accepted
- Context: BALANCE 9.6 allows only `modern-western` content overrides during stage-2 tuning. `modern-western` inherited every classic number and added wellbeing on top, which taxes each work session −3 and each lesson −2 against a weekly recovery of about +11. A classic week is 6–9 work sessions plus 3 lessons; in modern that is −24 to −33 a week, so the same week is not survivable. The pack had no `rules.json` at all, so there was no place for the difference to live.
- Options: 1) weaken the wellbeing deltas until a classic week fits (which deletes the system's point); 2) leave the deltas and accept a much longer modern game; 3) give `modern-western` its own economy: the same activities, priced for a week that must include rest.
- Decision: option 3, expressed as a `rules.json` overlay (deep-merged, so every untouched value still comes from `classic`). GDD 4.5's work, gig and study deltas are untouched. What moved, and why: **`wellbeing.relaxBase` 8 → 20, `relaxMax` 16 → 34, `driftStep` 2 → 5** so one rest session funds a week of work rather than a fifth of one; **`wellbeing.bands.collapse` 10 → 6** because at 10 a seat that dipped once collapsed in 96% of games; **`jobs.payPerSession` 8 → 30** because a modern week affords roughly a third of classic's sessions and rent, food and prices are inherited unchanged; **`happiness.decayPerWeek` 4 → 1 with `relaxBase` 2 → 7, `relaxMax` 6 → 11** because wellbeing is already modern's upkeep stat and ADR-0025's decay on top of it made happiness the binding goal in 62% of games; **`goals.educationPerDegree` 9 → 12** so the ladder is five degrees rather than six; **`wealthPointValue` 100 → 160** to spread the four goals apart. A `goals.careerDependabilityBp` override was tried and dropped: below 10000 the career goal caps under 100, which makes goals-100 unwinnable, so modern keeps classic’s 12500. Provisional ADR-0030 item prices were also tuned: smartphone $600 → $220 and laptop $900 → $420, which ADR-0030 explicitly left to this milestone. Modern event weights rose (going-viral 120 → 500, laptop-battery 150 → 400, cracked-screen 100/200 → 300/600, job-automated ×150 → ×500 of `automationRiskBp`), `phishing-scam` now tests cash plus bank rather than bank alone. `going-viral` keeps its smartphone condition: GDD 4.13 reads as gating the payout rather than the trigger, but removing the condition did not make the family fire either, and `modern-events.test.ts` asserts the trigger, so the condition stands and the miss is recorded in KI-008 instead.
- Consequences: `classic` is untouched — not one classic file changed, and its goldens and gates did not move. Modern goldens were regenerated. The modern ruleset is winnable: stalls 100% → 0%, median 46 weeks at goals 50 against classic's 40. Six of the twenty-six stage-2 targets remain unmet after the 15-iteration budget BALANCE 9.6 allows, and are recorded in `BALANCE_REPORT.md` and KI-008 with their achieved values rather than met (CLAUDE.md 1.5). The overlay is where any later modern tuning goes; a change to a classic number would have to justify itself against the frozen classic baseline instead.

## ADR-0036: Procedural music is plain WebAudio, not Tone.js

- Date: 2026-09-22
- Status: Accepted
- Context: AUDIO_SPEC 8.1 names Tone.js for the music layer, "loaded as a lazy chunk after first gesture". The rest of the same section is stricter than the library choice: all audio is synthesised at runtime, no audio files ship, and CLAUDE.md 1.3 forbids third-party assets and any network call at runtime. Tone.js is ~200 kB of JavaScript against a 350 kB gzip budget the app already spends 172 kB of, and the music this spec asks for is a pad, a lead on a weighted random walk and a two-note percussion layer — four oscillators and a filter.
...
- Consequences: modern sets 5 weeks a degree. Studying first becomes a real route to the career goal (probe: StudyFirst 5% → 32% wins, together with the AI now paying rent debt from the bank). Ordinary seats take their first job before any degree, so the credit mostly reaches them when they are hired again after a firing or a collapse, which it softens a little.
