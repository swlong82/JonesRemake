...
- Release (M8): PR #18 was squash-merged as 03caf07, which is therefore the first `main` commit containing the M8 gate; `m8` and `v1.0.0` belong there. `tools/retag-milestones.sh` now pushes them with `m1`–`m7`.
