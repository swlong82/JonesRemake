# REPLACE ME — `entitlements`

- **Contract:** `index.ts` exports the interface (see `docs/ROADMAP_SCAFFOLDS.md` §16.1, §16.4).
- **v1 default:** AllUnlockedEntitlements.
- **Replace by:** store adapter.
- **Contract guard:** `contract.test.ts` — has(feature) true for all in v1. Any new implementation must pass the same suite; add it to the
