...
- [~] M5.6 Delivery, subscriptions (billing, drift, retention dialog, cancel location rule), rent hikes, co-living quirks — note: subscriptions done (`modules/subscriptions.ts`: seven subs in `modern-western`, weekly billing from bank then cash, lapse on non-payment with the pack's happiness cost, price drift on the pack schedule, cancel only at the desk you signed up at; `grantsOf`/`hasGrant` is how other modules ask what a seat is subscribed to). Delivery, rent hikes and co-living still to do; the retention dialog is UI (M6.5).
